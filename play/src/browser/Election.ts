...
Election.condorcet = function (model, options) {
  ...
  if (topWinner) {
    ...
    text +=
      _icon(topWinner) + " beats all other candidates in one-on-one races.<br>";
    text += "</span>";
    text += "<br>";
    text +=
      "<b style='color:" + color + "'>" + topWinner.toUpperCase() + "</b> WINS";
  } else {
...
Election.irv = function (model, options) {
  ...
  text += "<br>";
  text +=
    "<b style='color:" + color + "'>" + finalWinner.toUpperCase() + "</b> WINS";
  model.caption.innerHTML = text;
};
...
// A proportional bar of seats (for Party List, where there can be hundreds).
var _seatBar = function (model, seatsWon, totalSeats) {
  ...
  for (var i = 0; i < model.candidates.length; i++) {
    ...
    bar += "<span style='width:" + pct + "%;background:" + c.fill + "'></span>";
  }
  bar += "</div>";
  return bar;
};
...
var _sortedScores = function (scores) {
  ...
  for (var id in scores) {
    rows.push({ id: id, score: scores[id] });
  }
...
var _spavScoreRows = function (round) {
  ...
  if (rows.length > limit) {
    text += "<div class='spav-more'>+" + (rows.length - limit) + " more</div>";
  }
  text += "</div>";
  return text;
};
...
Election.spav = function (model, options) {
  ...
  for (var s = 0; s < result.rounds.length; s++) {
    ...
    text += "<b>seat " + round.seat + ":</b> " + _icon(round.winner) + " wins";
