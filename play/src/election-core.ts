...
export type StvTransfer = {
  to: CandidateId | "elected" | "exhausted";
  value: number;
};

export type StvFlow = {
  action: "elected" | "eliminated";
  from: CandidateId;
  votes: number;
  ...
  while (continuing.length > 0) {
    ...
    for (const rank of rankings) {
      const first = rank.find(
        (candidate) => continuing.indexOf(candidate) >= 0,
      );
...
export const stv = (
  ...
  const flowRows = (
    flow: Record<CandidateId | "elected" | "exhausted", number>,
  ): StvTransfer[] =>
    Object.keys(flow)
      .map((to) => ({ to: to as StvTransfer["to"], value: flow[to] }))
  ...
  while (elected.length < seats && guard < 200) {
    ...
    const continuingIds = Object.keys(continuing).filter(
      (id) => continuing[id],
    );
    ...
    if (topVotes >= quota) {
      ...
      const flow: Record<CandidateId | "elected" | "exhausted", number> = {
