...
test("irv transfers votes without mutating input rankings", () => {
	...
	expect(result.rounds.map((round) => round.eliminated).filter(Boolean)).toEqual(["b"]);
	expect(ballots).toEqual(original);
});

test("party list uses Sainte-Lague divisors to allocate seats", () => {
...
test("stv elects by quota and fills remaining seats", () => {
	...
	expect(result.elected).toEqual(["a", "c"]);
	expect(result.rounds.map((round) => round.action)).toEqual(["elected", "eliminated", "last-seats"]);
});
